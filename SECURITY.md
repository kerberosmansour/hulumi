...
Current Hulumi tarballs carry a GitHub Artifact Attestations provenance record
generated by the reusable `.github/workflows/sign-and-publish.yml` signing
workflow with `actions/attest-build-provenance` v4. To verify:
...
signing workflow path, and workflow run URL. Repeat for
`@hulumi/policies@1.3.2`, `@hulumi/drift@1.3.2`,
...
1. **Exact-version-pinning with integrity hashes**: every `@pulumi/*` dep is
   ...
   The guard derives the expected current version from exact package manifests
   and the integrity hash from `pnpm-lock.yaml`, so cooled Dependabot Track B
   bumps can pass without a second CI-triggering commit while still proving
   exact pins + lockfile integrity.
...
- Every release builds tarballs and SBOMs without OIDC signing permission,
  then calls `.github/workflows/sign-and-publish.yml` to run
  `actions/attest-build-provenance` for SLSA Build L3 attestation, plus npm
  `--provenance` for the second Sigstore signature.
