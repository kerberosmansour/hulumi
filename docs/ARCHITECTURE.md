...
| `@hulumi/platform-patterns` (`packages/platform-patterns/src/`)                                | Cross-provider deployment patterns — `CloudflareOriginIngress`, `GitHubAwsOidcDeploymentRole`, `DeploymentRepositoryFoundation`, `BuildProvenanceFoundation`, `PulumiStateBackendFoundation`, `RunnerGovernanceFoundation`.                                                                                    |
| `@hulumi/k8s-baseline` (`packages/k8s-baseline/src/`)                                          | Hardened K8s/EKS/Istio Pulumi `ComponentResource`s — `EksClusterFoundation`, `HardenedHelmRelease`, `EksSubnetTagger`, `IstioFoundation`, `AlbMeshedHttpEntrypoint`, `KubernetesSecretFromAwsSecretsManager`, `RdsCredentialSecret`, `GitHubAppCredential`, runtime detection, backup, and add-on foundations. |
...
| `packages/baseline/src/aws/aws-organization-security-foundation.ts`                            | `AwsOrganizationSecurityFoundation` — organization delegated admins, central Security Hub/Config posture, S3 account block, and bounded SCP guardrails                                                                                                                                                         |
| `packages/baseline/src/aws/secure-aws-primitives.ts`                                           | `SecureIamDeploymentRole`, `SecureWorkloadRole`, `SecureSecret`, and `SecureLaunchTemplate` — hardened IAM, secret, and compute metadata primitives                                                                                                                                                            |
| `packages/baseline/src/aws/security-detection-foundation.ts`                                   | `SecurityDetectionFoundation` — composes IdentityAlarms plus finite EventBridge alarm families for org, state, EKS control-plane, CloudTrail/KMS/Config, and security-service disablement detection                                                                                                            |
| `packages/platform-patterns/src/runner-governance-foundation.ts`                               | `RunnerGovernanceFoundation` — GitHub environment, OIDC, and finite self-hosted runner approval contract for privileged deployment workflows                                                                                                                                                                   |
...
| `@hulumi/policies` (`packages/policies/src/`)                                                  | CrossGuard PolicyPack module for AWS, GitHub, K8s/EKS, Cloudflare edge, origin-bypass, deployment-governance, runner-governance, and workflow-governance checks.                                                                                                                                               |
| `packages/policies/src/aws/hulumi-hardening-pack.ts`                                           | `HulumiHardeningPack` — H1–H5 plus `STATE-1`, `PRIM-1` through `PRIM-4`, and `DETECT-1` through `DETECT-3` detection invariants                                                                                                                                                                                |
| `packages/policies/src/aws/cis-v5-pack.{ts,rules.ts}`                                          | `CisV5Pack` — CIS AWS Foundations v5.0.0 sections 1–3                                                                                                                                                                                                                                                          |
| `packages/policies/src/aws-org/hulumi-aws-org-hardening-pack.ts`                               | `HulumiAwsOrgHardeningPack` — organization delegated-admin, SCP, role-separation, and S3 account-block invariants                                                                                                                                                                                              |
...
| `@hulumi/drift` (`packages/drift/src/`)                                                        | Local-first drift classifier with TLA+-mirrored verdict matrix plus the read-only `hulumi validate live` posture-artifact CLI                                                                                                                                                                                  |
...
| `packages/drift/src/live-validator.ts`                                                         | Bounded live-validation runner, finding schema, JSON/Markdown/SARIF renderers, CLI parser, and GitHub runner-governance evaluator                                                                                                                                                                              |
...
- **`hulumi validate live`** — `@hulumi/drift` CLI entry point that turns read-only live posture adapters into JSON, Markdown, and SARIF artifacts.
...
Engineer
  ...
  └─prompts→ Claude Code
                ...
                └─writes Pulumi program in user repo
                        │
                        ├─imports @hulumi/baseline.aws.{SecureBucket,AccountFoundation,AwsOrganizationSecurityFoundation,SecureIamDeploymentRole,SecureSecret,SecureLaunchTemplate,SecurityDetectionFoundation}
                        ├─imports @hulumi/platform-patterns.PulumiStateBackendFoundation for S3/KMS state backend posture
                        ├─imports @hulumi/platform-patterns.RunnerGovernanceFoundation for GitHub deployment runner posture
                        └─imports @hulumi/policies packs for AWS/account/org checks
                                    │
...
Live posture validation (separate advisory path):
  schedule/manual CI ─hulumi validate live→ bounded provider adapters
                                      ├─ aws-org
                                      ├─ pulumi-state
                                      ├─ eks
                                      └─ github
                                          │
                                          └─ deterministic findings → JSON / Markdown / SARIF artifacts
```
