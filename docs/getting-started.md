...
**`PolicyPack` constructor exits with `process.exit(1)`.** `@pulumi/policy`'s `new PolicyPack()` starts a gRPC server at module load time and only one is allowed per process. Import `HulumiHardeningPack` and `CisV5Pack` from their dedicated entrypoints under `@hulumi/policies/aws/packs/*`, never side-by-side from the same file. See [docs/lessons/hulumi-m2.md](./lessons/hulumi-m2.md) for the underlying constraint.
