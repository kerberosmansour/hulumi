  ...
  implements WorkloadCapabilityIssuerBoundaryOutputs
{
  ...
  constructor(
    ...
    const awsVpcPodEniToleration =
      't.key == "vpc.amazonaws.com/pod-eni" && t.operator == "Exists" && !has(t.value) && t.effect == "NoSchedule" && !has(t.tolerationSeconds)';
    const exactTolerations = `has(object.spec.tolerations) && (object.spec.tolerations.size() == 1 || object.spec.tolerations.size() == 2 || object.spec.tolerations.size() == 3 || object.spec.tolerations.size() == 4) && object.spec.tolerations.exists(t, ${configuredToleration}) && (object.spec.tolerations.size() == 1 || object.spec.tolerations.size() == 2 || (object.spec.tolerations.exists(t, ${notReadyToleration}) && object.spec.tolerations.exists(t, ${unreachableToleration}))) && (object.spec.tolerations.size() == 1 || object.spec.tolerations.size() == 3 || object.spec.tolerations.exists(t, ${awsVpcPodEniToleration})) && object.spec.tolerations.all(t, (${configuredToleration}) || (${notReadyToleration}) || (${unreachableToleration}) || (${awsVpcPodEniToleration}))`;
    ...
    const exactEnvelope = exactEnvironment.apply(
      (environmentExpression) =>
        ...
        )} && (!has(m.subPath) || m.subPath == "") && m.readOnly == true) && object.spec.containers[0].volumeMounts.exists(m, m.name == "tmp" && m.mountPath == "/tmp" && (!has(m.subPath) || m.subPath == "") && (!has(m.readOnly) || m.readOnly == false)) && (!has(object.spec.containers[0].lifecycle)) && (!has(object.spec.containers[0].livenessProbe)) && (!has(object.spec.containers[0].readinessProbe)) && (!has(object.spec.containers[0].startupProbe)) && has(object.spec.containers[0].resources) && dyn(object.spec.containers[0].resources).requests.cpu == "100m" && dyn(object.spec.containers[0].resources).requests.memory == "128Mi" && dyn(object.spec.containers[0].resources).limits.cpu == "1" && dyn(object.spec.containers[0].resources).limits.memory == "512Mi" && ${exactPlacement}`,
    );
